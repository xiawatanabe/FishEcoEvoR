...
- エネルギー収支をサイズの関数として表現し、消費率(consumption rate)から成長・再生産率を決める
...
- このエネルギー収支は個体のなかの質量とエネルギーの流れのすべてを考慮する。成長と再生産に使われるエネルギーと同様、消化と代謝による損失も扱う。
...
- 同化作用(anabolic)と異化作用(catabolic)のプロセスから構成される
  ...
  - $kw$  :エネルギーの損失(losses, catabolic)　</dev>
...
# 体長ベースの成長式：vBモデル
<div  class = "box">
(3.4)式は$w=cl^3$の関係を使って以下の様にかける(Box3.1)
$$
...
$$
K：growth constant（時間あたり）, $L_\infty$：asymptotic length: 極限体長
...
<div aline = "left"> <span fontsize = 18pt>
...
## 図3.2　Kと$L\infty$　の関係
```{r 3.2, fig.height=3, fig.width =6}
plotKvsLinf()
```
...
このときの関係は以下のようにかける
$$
W_\infty=\bigl(\frac{A}{k}\bigr)^\frac{1}{1-n}
$$
初期の成長は傾き$KL_\infty$の直線似近く（斜めの破線）、$l$が$L_\infty$に近づくと成長が止まる（横の破線）。

## 図3.1b
```{r 3.1, fig.height=3, fig.width =6}
plotSketch_and_vonB()
```
...
$$
\frac{dw}{dt}=Aw^n\bigl[1-\bigl(\frac{w}{W_\infty}\bigr)^{1-n}\bigr]
$$
...
基礎代謝（$M_{std}$  Standatd metabolism） は、生命を維持するために必要な基本的な代謝量であり、

$M_{std} = k_sw^n$ と書ける。一方、activity metabolism($M_{act}$)は見積もりが難しい。ここでは簡単のため$M_{std}$に比例するとしその比率は$k_s$に含まれると考える。すると$M_{std} + M_{act}$は、最大消費レベル$hw^n$に比例すると考えることができるので$k_s = f_c\varepsilon_ah$ と書ける。$f_c$は限界摂餌レベル（これ以下に減らすと生命を維持できないレベル）である。
$$
...
　成長式に含まれるいろいろなプロセスは、$W_\infty$できまるとしているが、たとえば3.11式からは$W_{\infty}$がAとkで決まるともいえる。Aを一定と考えれば、kをMater traitとし、Aとkから$W_{\infty}$を決めることもできる。なぜそうしないか？

- 使いやすく推定が楽。
- data-limitedな資源でも推定可能→data-poorな資源にもモデルを応用できる。
- 直感的（intuitive)にわかりやすい。$W_{\infty}=10kg$といわれるとそりゃ魚食性魚類だろうと想像できるが、$k=0.44/yr$といわれてもピンとこない。

　本章の個体成長モデルは、生活史に関わる多くの重要なパラメータが$W_{\infty}$　と（または） Aに比例するから、Aと$W_\infty$ が決まればその他のパラメータを決めることができる。理論的には正しいが、データ解析からは明確にその関係を示せていない。$W_\infty$ と$W_m$ (成熟サイズ) の比（$\eta_m$)は種によってかなり異なるが、これをinvariantsといえるだろうか？　この疑問には3つの観点から答えることができよう、semantic（意味論的）、empiric(経験的)、thoretic(理論的)に。

　ある形質（$W_\infty$と$ W_m$ の比など）が”invariant”ということは、種によって異ならないということではなく、その形質がほかの形質（成長速度、水温などの環境、系統発生学的な位置関係）と”co-vary”ではないことを意味する。現段階では、invariantとされる形質の種による変動はランダムで、他の形質との関係で説明できない予測不能であるということだ。生活史パラメータが基本的にinvariantであるという誤解をさけるためには、life history parameterといったほうがよかろう。

　経験的には、データに限界があることを忘れてはいけない。パラメータのいくつかは直接推定されていない。たとえばAであるが、これは個々の推定されたvon Bertalanffy の成長曲線から推定したKと$L_{\infty}$から得たもので、考慮していないバイアスが含まれるだろうし、systematicなバイアスもあるだろう。

　それに成長曲線の多くは１個体のそれではなく、年齢あたりサイズの平均である。もし速い成長が高い死亡率につながるのであれば、高齢になるほどデータは遅い成長の個体のもので占められ、結果にバイアスが生じる。再生産投資などは推定自体が難しい。生殖腺重量は繁殖投資のプロキシではあるが、年間の繁殖投資を推定するには、一年に何回産卵するか、摂密度によって産卵はどのように変化するか、skip spawnerがどれくらいいるか、産卵回遊や産卵行動にどれくらいの投資が必要か、など知る必要がある。これらが適切に考慮されたとしても理論値からの残差は十分説明できないだろう。

　きっと考慮できていない形質で説明される変動があるのだ。それが図3.3にみられるような・・・成長速度の10倍の変動幅をもたらすような。それがなにかはわからないが、仮説はたてられる。候補になるtrait axisの一つは　slow-fast life history continuum である。

成長がおそい→餌が取れない＆clearance rateが小さい→活動量（activity）が小さい→$f_c$ (個体維持に最低限必要な摂餌レベル)が小さい（逆に成長が速いと$f_c$が大きくなる）

　→$f_c$は定数でなく、A($=\varepsilon_ah(f_0-f_c)$)も変わる。

clearance rate を正確に推定するのは難しいし、Niel Gerner Andersonの言葉を借りれば　enegy budgetのダークホースである活動量（activity coefficient)についての知識も乏しい。実際にトレードオフが明確に観測されていないのはこのような知識の乏しさによるだろう。

　もうひとつの候補になる形質は防御への投資である。身を守るための棘を持ったり、擬態したり、隠れたりといった戦略にはそれぞれコストがかかる。隠れるということはその間、餌を得ることを我慢しなくてはならない。そのためのコストもある。防御形質が持つトレードオフは、slow-fast life　history continuumよりさらに扱いにくい。防御への投資と死亡率のトレードオフは後の章であつかう
...
この章では vBモデルをもとにしたbiphasic growth modelを構築した。このモデルは成長と再生産（繁殖）が極限サイズが異なる魚種間でどのように変化するかを示す：極限サイズが大きい魚種は、サイズあたりの繁殖投資が少ない。
...
para2
本章の成長モデルはvBのモデルとは一致しない。とくにJuvenile stageでの予測値はvBの予測値より小さくなる。vBモデルは成長データをよく代表するものとして受け入れられてきたので残念である。この違いは、吸収と基礎代謝による損失においてn=3/4を用いたことに由来する。この選択は、代謝がサイズによってどのようにスケーリングされるかという現在の認識に従うと（つまりスケーリングに関する現在の研究の流れからすると）完璧ではない。基礎代謝のスケールは3/4より大きいという報告がある（Killen, et al. 2007). 活動(activity)のスケールは実測が難しいが、理論的に3/4より大きいという考察がなされている（Ware1978）。juvenile stageのスケーリング指数を3/4より高くすれば、BP成長モデルの推定値はよりvBモデルに近くなる。したがって、BP成長モデルを改善するならば、明示的に代謝によるエネルギーの損失の項を設け、獲得の項とは異なるスケールを与えることになろう。あるいは、スケールを増やしたくない（nとも１とも異なるスケールを加えたくない）ならば、代謝による損失を catabolic termに含めることになろう。
...
本章のモデル構築での選択は、以下に述べる私の意図に沿うものである。つまり、筋の通った一般性のある理論を構築するため、ある程度精度を犠牲にしている。この犠牲によって成長と再生産を極限サイズの関数として表現することが可能になり、極限サイズの変動から様々な魚類資源について一般性のある記述が可能になる。この恩恵は、今後の章で受けることになる。そこでは形質ベースの成長と再生産のモデルによって、様々な魚類資源に対する漁業のサイズ依存的なインパクトを明らかにできる。結論として3.18式と3.30式は完璧ではない。しかしシンプルなサイズベースの成長式を構築し、すべての変動（変異性）をMaster traitとしての極限体長で示そうという目的を、もっともよく果たすものである。
