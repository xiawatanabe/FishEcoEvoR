---
title: "Individual Growth and Reproduction"
...
- Size spectrum theoryを集団にまで発展させるには、個体について詳細な記述が必要
- 本章の問い：個体は摂餌から得られるエネルギーをどう利用するか？
→個体のサイズの関数としてエネルギー収支（enegy budget)を表現し、摂餌率（consumption rate)から成長率（growth rate）と再生産率（reproduction rate)を決定する。

- 魚類の多様な生態を表現するには多くのパラメータが必要だが、シンプルな理論の構築には邪魔
- Lige-history invariants を利用してパラメータを減らしていき、最終的に種によって変化するパラメータは一つに絞られたー極限サイズ（W∞）である。これは生活史の多様さを特徴づけるmaster trait の役割を果たすパラメータである。
- エネルギー収支とは、個体の中での質量とエネルギーの流れである。
assmilation losses, metabolic losses, enegy spent on growth and reproduction

* 生活史の不変定数（Life History Invariants）とは，生活史におけるキーになる無次元の重要パラメータが異なる種間系統間で一定値をとる場合のそのパラメータをいう．


## 3.1 The von Bertallanffy growth model
(para1) biphasic growth model を開発
成熟前と成熟後の異なる成長様式を記述
- 成熟前：エネルギーをすべて成長に使う
- 成熟後：エネルギーを成長と再生産に使う
再生産にかかるコストを考慮した成長モデル→のちの章で必要になる。
最初にもとになるvon Bertalanffyの成長式を詳しく見ていく

(para2)　von Bertalanffyの成長式は同化作用(anabolic)と異化作用(catabolic)のプロセスから構成される。

$$ \frac{dw}{dt}= Aw^n- kw $$

Aw^n:エネルギーの獲得(accuisition) 
kw  :エネルギーの損失(losses)　　　
いずれも体重のべき乗に比例する（それぞれnと1）

nについて

von Bertalanffy はエネルギーの獲得は同化作用の過程によって限定されると考えた。
